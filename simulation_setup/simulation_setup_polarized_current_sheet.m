...
matVixBx = mfVixBx(0,0,Z); if isscalar(matVixBx); matVixBx = repmat(matVixBx,nz,nx); end
matVixBy = mfVixBy(0,0,Z); if isscalar(matVixBy); matVixBy = repmat(matVixBy,nz,nx); end
matVixBz = mfVixBz(0,0,Z); if isscalar(matVixBz); matVixBz = repmat(matVixBz,nz,nx); end
...
matVexBx = mfVexBx(0,0,Z); if isscalar(matVexBx); matVexBx = repmat(matVexBx,nz,nx); end
matVexBy = mfVexBy(0,0,Z); if isscalar(matVexBy); matVexBy = repmat(matVexBy,nz,nx); end
matVexBz = mfVexBz(0,0,Z); if isscalar(matVexBz); matVexBz = repmat(matVexBz,nz,nx); end

fN = symfun(n,[x y z]);
mfN = matlabFunction(fN);
matN = mfN(0,0,Z); if isscalar(matN); matN = repmat(matN,nz,nx); end

fPi = symfun(PTi,[x y z]);
mfPi = matlabFunction(fPi);
matPi = mfPi(0,0,Z); if isscalar(matPi); matPi = repmat(matPi,nz,nx); end
...
matEx = mfEx(0,0,Z); if isscalar(matEx); matEx = repmat(matEx,nz,nx); end
matEy = mfEy(0,0,Z); if isscalar(matEy); matEy = repmat(matEy,nz,nx); end
matEz = mfEz(0,0,Z); if isscalar(matEz); matEz = repmat(matEz,nz,nx); end
...
if 1 % Vy, i, e,
  hca = h(isub); isub = isub + 1;
  plot(hca,zvec,matViy,zvec,matVey);  
  hca.YLabel.String = 'Vy';
  legend(hca,{'viy','vey'})
end
if 1 % GradPiz
  hca = h(isub); isub = isub + 1;
  plot(hca,zvec,matGradPiz);  
  hca.YLabel.String = 'grad Piz';
end
if 1 % VxB_z
  hca = h(isub); isub = isub + 1;
  plot(hca,zvec,matVixBz,zvec,matVexBz);  
  hca.YLabel.String = 'VxBz';
  legend(hca,{'i','e'})
end
if 0
  hca = h(isub); isub = isub + 1;
...
%h(1).YLim = [0 6];
hlinks.Targets(1).XLim = [zimin zimax];
