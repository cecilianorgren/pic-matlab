...
if 1
  ...
  diff_order = 1;
  dxTxx = [1*diff(Txx(1:2,:),1,1); diff(Txx,diff_order,1)];
  dxTxy = [1*diff(Txy(1:2,:),1,1); diff(Txy,diff_order,1)];
  dxTxz = [1*diff(Txz(1:2,:),1,1); diff(Txz,diff_order,1)];
  dyTyx = T.xx*0;
  dyTyy = T.xx*0;
  dyTyz = T.xx*0;
  dzTzx = [1*diff(Tzx(:,1:2),1,2), diff(Tzx,diff_order,2)];
  dzTzy = [1*diff(Tzy(:,1:2),1,2), diff(Tzy,diff_order,2)];
  dzTzz = [1*diff(Tzz(:,1:2),1,2), diff(Tzz,diff_order,2)];
