...
if 1 % z(t,x)
  ...
  pcolor(hca,pic.xi,pic.twci,sep_xz.z); 
  shading(hca,'flat')
  hb = colorbar('peer',hca);
  hb.YLabel.String = 'z_{sep} (d_{i0})';
  hold(hca,'on')  
  contour(hca,pic.xi,pic.twci,sep_xz.z,zlevels,'k');
  hold(hca,'off')
  hold(hca,'on')  
  plot(hca,sep_xz.xline_x,pic.twci,'k','linewidth',1.5);
  hold(hca,'off')  
...
if 1 % z(t,x)/x(t,x)
  ...
  pcolor(hca,pic.xi,pic.twci,z_over_x); 
  drawnow;
  clim = hca.CLim;
  shading(hca,'flat')
  hb = colorbar('peer',hca);
...
if 1 % ni(z=0)
  hca = h(isub); isub = isub + 1;
  pcolor(hca,pic.xi,pic.twci,nicold'); 
  drawnow;
  clim = hca.CLim;
  shading(hca,'flat')
  hb = colorbar('peer',hca);
