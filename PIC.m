...
%               end        
            ...
            if doSep
              hold(hca,'on')
              contour(hca,tmp_obj.xi(iAx),tmp_obj.zi(iAz),A(iAx,iAz)',sepA*[1 1],'k--')
              hold(hca,'off')
            end       
            if doTrajectories
              hold(hca,'on')
...
%       end
    ...
    function varargout = plot_map(obj,varargin)
      ...
      if doQuiv % Keep here for now, later might add different quivs for different panels
        quiv_vec_x = obj.get_exp(quivers{1});
        quiv_vec_z = obj.get_exp(quivers{2});
        [quiv_loc_x,quiv_loc_z] = ndgrid(obj.xi,obj.zi);
        quiv_vec_x = quiv_vec_x(1:stepQuiv:end,1:stepQuiv:end);
        quiv_vec_z = quiv_vec_z(1:stepQuiv:end,1:stepQuiv:end);
        quiv_loc_x = quiv_loc_x(1:stepQuiv:end,1:stepQuiv:end);
        quiv_loc_z = quiv_loc_z(1:stepQuiv:end,1:stepQuiv:end);
      end
      if doA
        A = obj.A;
        if numel(stepA) == 1
...
%       out = obj.mass(species)*obj.wpewce^2*(vxx + vyy + vzz)/3;
    ...
    function out = vzdzvy(obj,species)        
    ...
    function out = vdv(obj,comp,species)
      ...
      switch comp
        case 'x' % vdv_x = vx*dvx/dx + vz*dvx/dz
          vdivv = vx.*dxVx + vz.*dzVz;
        case 'y' % vdv_y = vx*dvy/dx + vz*dvy/dz
          vdivv = vx.*dxVy + vz.*dzVy;
        case 'z' % vdv_z = vx*dvz/dx + vz*dvz/dz
          vdivv = vx.*dxVz + vz.*dzVz;
      end
    ...
    function out = rc(obj,species,mode)
      %
      % wc = eB/m
      if not(exist('mode','var'))
        mode = 'total';        
    ...
    function out = rcx(obj,species)
      %
      % wc = eB/m            
      
      out = rc(obj,species,'xx'); % because we have included a 2 inside vt = sqrt(2T/m)
    end
    function out = rcy(obj,species)
      %
      % wc = eB/m            
      
      out = rc(obj,species,'yy'); % because we have included a 2 inside vt = sqrt(2T/m)
    end
    function out = rcz(obj,species)
      %
      % wc = eB/m            
      
      out = rc(obj,species,'zz'); % because we have included a 2 inside vt = sqrt(2T/m)
    end
    ...
    function varargout = divp(obj,species,varargin)
      ...
      np_smooth = 7;
      doSmooth = 0;
      doSubcomp = 0; % only include gradients of certain component
...
%               [dxTzy,dzTzy] = gradient(pyz,dx,dz);
    ...
    function out = dxPxy(obj,species)      
      nx = obj.nx;     
      nz = obj.nz;
      dx = obj.xi(2)-obj.xi(1);
      dy = Inf;
      ...
      pxy = obj.pxy(species);
      if obj.nt == 1
        [dzPxy,dxPxy] = gradient(pxy,dz,dx);
      else
        [dzPxy,dxPxy,~] = gradient(pxy,dz,dx,Inf);
      end
      out = dxPxy;
    end
    function out = dzPzy(obj,species)      
      nx = obj.nx;     
      nz = obj.nz;
      dx = obj.xi(2)-obj.xi(1);
      dy = Inf;
      dz = obj.zi(2)-obj.zi(1);
      pyz = obj.pyz(species);
      [dzPzy,dxPzy] = gradient(pyz,dz,dx);
      out = dzPzy;
    end
    function out = dxPexy(obj)      
      species = find(obj.get_charge == -1);
      out = dxPxy(obj,species);
    end
    function out = dzPezy(obj)      
      species = find(obj.get_charge == -1);
      out = dzPzy(obj,species);
    end
    ...
    function out = rot(obj,str1,str2,str3) % Rotation
      warning('Pleasy verify rotation a bit more carefully.')
      % Calculate rotation of specified strings
      % out = obj.rot('vex','vey','vez')
      % Rx = ddy vz - ddz vy
      % Ry = ddz vx - ddx vz
      % Rz = ddx vy - ddy vz
      
      var1 = obj.get_exp(str1); 
      var2 = obj.get_exp(str2); 
      var3 = obj.get_exp(str3); 
      
      dx = obj.xi(2)-obj.xi(1);
      dy = Inf;
      dz = obj.zi(2)-obj.zi(1);
      
      dvy_dx = [diff(var2,1,1); var2(end-1,:)-var2(end,:)];
      dvz_dx = [diff(var3,1,1); var3(end-1,:)-var3(end,:)];
      dvx_dy = 0;
      dvz_dy = 0;
      dvx_dz = [diff(var1,1,2), var1(:,end-1)-var1(:,end)];
      dvy_dz = [diff(var2,1,2), var2(:,end-1)-var2(:,end)];

      rot_x = (dvz_dy/dy - dvy_dz/dz);
      rot_y = (dvx_dz/dz - dvz_dx/dx);
      rot_z = (dvy_dx/dx - dvx_dy/dy);
    ...
    function out = rotve(obj,comp)
      res = obj.rot('vex','vey','vez');
      out = res.(comp);
    end
