%% haggerty2015_comparison
% Calculate the parallel potential in the exhaust
tag = 'no02m';
twci = 120;
xlim = [60 100];
zlim = [-1 10];
pic = no02m.xlim(xlim).zlim(zlim).twcilim(twci,'exact');
field = 'B';
...
z0_all = [2:0.2:10];
%z0_all = [1:0.2:1.8];
x0_all = 61+zeros(numel(z0_all),1);
nlines = numel(z0_all);
...
for iline = 1:nlines
  ...
  intEpar_all(iline).phi = intEpar_along_fieldline;
  intEpar_all(iline).l = linearc;
